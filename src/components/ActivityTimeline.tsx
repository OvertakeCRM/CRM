        ...
        <li key={entry.id} className="flex gap-3">
          ...
          <div className="min-w-0 flex-1 pb-1">
            ...
            {entry.type === "quick_log" && (
              <p className="mt-0.5 text-sm text-slate-600">
                <span className="rounded bg-slate-100 px-1.5 py-0.5 font-medium">{entry.note}</span>
              </p>
            )}
            ...
            {entry.type === "stage_change" && entry.note && (
              <p className="mt-0.5 whitespace-pre-wrap text-sm text-slate-500 italic">&ldquo;{entry.note}&rdquo;</p>
            )}
          </div>
        </li>
      ))}
